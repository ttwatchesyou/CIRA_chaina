...
import { cancelTrainingJob, TrainingServiceError } from "@/server/services/training.service";
...
export async function POST(_request: Request, { params }: RouteContext) {
  ...
  try {
    const job = await cancelTrainingJob(jobId, user.id);
    ...
    return apiError("TRAINING_CANCEL_FAILED", "ยกเลิกงานไม่สำเร็จ", 500);
  }
}
