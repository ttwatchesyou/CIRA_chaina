...
1. แก้ค่า `PUBLIC_APP_URL` ในไฟล์ `.env` ให้เป็น IP ของคอมพิวเตอร์ที่มือถือเข้าถึงได้
2. ห้ามใช้ `localhost` เพราะในมือถือคำนี้หมายถึงมือถือเอง
3. หยุดและเปิดเว็บใหม่หลังแก้ `.env`
...
- `PUBLIC_APP_URL` ไม่ใช่ `localhost`
- IP ใน `.env` เป็น IP ปัจจุบันของคอมพิวเตอร์
- Firewall อนุญาตพอร์ต 3000
- เปิดเว็บใหม่หลังแก้ `.env`
